...
2. 如果 `grok-search.sh` 或 `dual-search.sh` 中的 Grok 分支出现 `HTTP 429/5xx`、`timeout`、`upstream_error`、`Cloudflare` 相关错误：
- 立即回退到 `tavily-search.sh` 继续完成当前任务。
- 不得因为 Grok 失败而中止整个回答。
3. 如果 `tavily-search.sh` 成功且 `grok-search.sh` 失败：
- 先输出 Tavily 的可验证结论与来源。
- 明确注明：`Grok 通道暂时不可用，已自动回退 Tavily`。
4. 若两者都失败：
- 输出失败原因摘要。
- 提供下一步最小可执行动作（例如：补充 key、重试时间窗、改用 `web-fetch.sh` 抓指定 URL）。
5. 对关键事实，优先补一轮 `web-fetch.sh` 抓取原文并二次确认。
